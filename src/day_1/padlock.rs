...
fn parse_line_to_rotation_tracked(l: String) -> (Rotation, usize) {
    let length = l.len();
    ...
    let offset: (usize, usize) = match length {
        0..2 => {
            panic!("Got a bad line")
        }

        2 => match l.chars().nth(1) {
            Some(x) => (x.to_digit(10).unwrap() as usize, 0),
        ...
        4.. => {
            let number: usize = l[1..].parse().unwrap();
            let rotations = number / 100;
            let upper = get_tens_value(number);
            let lower = get_ones_value(number);
            (upper * 10 + lower, rotations)
        }
    };
...
pub fn parse_day_1_input_tracked(f_name: &str) -> Result<Vec<(Rotation, usize)>> {
    ...
    let lines = reader
        ...
        .map(|l| {
            if let Ok(line) = l {
                parse_line_to_rotation_tracked(line)
            } else {
                println!("Got bad input");
                panic!("Bad input");
            }
        })
        .collect::<Vec<(Rotation, usize)>>();
...
impl Padlock {
    ...
    pub fn get_next_location_tracked(&self, rot: &Rotation) -> (usize, bool) {
        match rot.dir {
            Direction::L => match self.current.checked_sub(rot.offset) {
                ...
                None => match rot.offset.checked_sub(self.current) {
                    Some(of) => (self.dial.len() - of, true),
                    None => {
                        println!("We shouldn't be getting here! We've failed on both directions");
                        panic!("Failed in rotating left");
                    }
                },
            },
            Direction::R => match upper_clamp(self.current + rot.offset, 99) {
                Some(loc) => (loc, false),
                None => {
                    let to_zero = 99 - self.current;
                    let remaining = rot.offset - to_zero;
    ...
    pub fn rotate_tracked(&mut self, rot: &Rotation) -> bool {
        let start = self.current;
        let (next, hit_zero) = self.get_next_location_tracked(rot);
        self.current = next;
        if hit_zero {
            println!(
                "Starting at {} we hit zero when performing an {:?} for {} rotations",
                start, rot.dir, rot.offset
            );
        }
        hit_zero
    }
    pub fn evaluate(&mut self, ops: &[Rotation]) -> u32 {
        let mut acc = 0;
        for op in ops {
            self.rotate(op);
            if self.current == 0 {
...
mod tests {
    ...
    fn test_get_next_location() {
        let padlock = Padlock::new();
        let rot = Rotation::new(Direction::R, 50);
    ...
    #[test]
    fn test_eval_test_input() -> Result<()> {
        let mut padlock = Padlock::new();
        let rotations = parse_day_1_input("input/day_1_test_input.txt")?;
        let total_zeros = padlock.evaluate(&rotations);
        println!("Total Zeroes In Test Input: {total_zeros}");

        Ok(())
    }
    #[test]
    fn test_eval_input() -> Result<()> {
        let mut padlock = Padlock::new();
        let rotations = parse_day_1_input("input/day_1_input.txt")?;
        let total_zeros = padlock.evaluate(&rotations);
        println!("Total Zeroes In Input: {total_zeros}");
        Ok(())
    }
    #[test]
    fn test_eval_test_input_tracked() -> Result<()> {
        let mut padlock = Padlock::new();
        let rotations = parse_day_1_input_tracked("input/day_1_test_input.txt")?;
        let total_zeros = padlock.evaluate_tracked(&rotations);
        println!("Total Zeroes In Tracked Test Input: {total_zeros}");

        Ok(())
    }
    #[test]
    fn test_eval_input_tracked() -> Result<()> {
        let mut padlock = Padlock::new();
